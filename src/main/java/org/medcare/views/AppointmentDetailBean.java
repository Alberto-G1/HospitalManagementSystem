...
public class AppointmentDetailBean implements Serializable {
    ...
    /**
     * This method is called after the view parameter 'id' is set.
     * It loads the appointment for editing or creates a new one.
     */
    public void loadAppointment() {
        if (appointmentId == 0) { // This means we are creating a new appointment
            isNewAppointment = true;
            appointment = new Appointment();
            appointment.setDate(LocalDate.now());
            appointment.setTime(LocalTime.of(9, 0)); // Default time
            appointment.setStatus(AppointmentStatus.SCHEDULED);
        } else {
            isNewAppointment = false;
            appointment = appointmentService.getById(appointmentId);
            if (appointment == null) {
                // Handle the case where an invalid ID is passed
                addMessage(FacesMessage.SEVERITY_ERROR, "Error", "Appointment not found.");
            }
        }
